...
class RegexAnalyzer
{
    ...
    protected static function char_code( $c )
    {
        return ord($c[0]);
    }
    protected static function char_code_range( $s )
    {
        $l = is_array($s) ? count($s) : strlen($s);
        return array(ord($s[0]), ord($s[$l-1]));
    }
    protected static function character_range($first, $last=null)
    {
    ...
    protected static function concat($p1, $p2)
    {
        if ( $p2 )
        {
    ...
    protected static function match_chars( $CHARS, $s, $pos=0, $minlen=1, $maxlen=INF )
    {
    ...
    protected static function match_char_range( $RANGE, $s, $pos=0, $minlen=1, $maxlen=INF )
    {
        $lp = $pos; $l = 0; $sl = strlen($s);
    ...
    protected static function match_char_ranges( $RANGES, $s, $pos=0, $minlen=1, $maxlen=INF )
    {
        $lp = $pos; $l = 0; $sl = strlen($s);
    ...
    protected static function punct( )
    {
        return self::$PUNCTS[rand(0, count(self::$PUNCTS)-1)];
    }
    protected static function space( $positive=true )
    {
        if ( false !== $positive )
        {
    ...
    protected static function digit( $positive=true )
    {
        if ( false !== $positive )
        {
    ...
    protected static function alpha( $positive=true )
    {
        if ( false !== $positive )
        {
    ...
    protected static function word( $positive=true )
    {
        if ( false !== $positive )
        {
    ...
    protected static function any( )
    {
        return self::$ALL[rand(0, count(self::$ALL)-1)];
    }
    protected static function character( $chars, $positive=true )
    {
        if ( false !== $positive )
        {
            $l = count($chars);
    ...
    protected static function random_upper_or_lower( $c )
    {
        return rand(0,1) ? strtolower($c) : strtoupper($c);
    }
    protected static function case_insensitive( $chars, $asArray=false )
    {
        if ( $asArray )
        {
    ...
    protected static function walk( $ret, $node, $state )
    {
    ...
    protected static function map_src( $ret, $node, $state )
    {
        $type = $node->type;
    ...
    protected static function map_any( $ret, $node, $state )
    {
        $type = $node->type;
    ...
    protected static function map_min( $ret, $node, $state )
    {
        $type = $node->type;
        if ( Regex::T_ALTERNATION === $type )
        {
    ...
    protected static function map_max( $ret, $node, $state )
    {
        $type = $node->type;
        if ( Regex::T_ALTERNATION === $type )
        {
    ...
    protected static function map_1st( $ret, $node, $state )
    {
        $type = $node->type;
    ...
    protected static function reduce_len( $ret, $node, $state )
    {
        if ( isset($state->ret) )
        {
    ...
    protected static function reduce_str( $ret, $node, $state )
    {
        if ( isset($state->ret) )
        {
            $ret .= $state->ret;
    ...
    protected static function reduce_src( $ret, $node, $state )
    {
        if ( isset($state->ret) )
        {
    ...
    protected static function reduce_peek( $ret, $node, $state )
    {
        if ( isset($state->ret) )
        {
    ...
    protected static function match_hex( $s )
    {
        $m = false;
        if ( (strlen($s) > 2) && ('x' === $s[0]) )
        {
    ...
    protected static function match_unicode( $s )
    {
        $m = false;
        if ( (strlen($s) > 4) && ('u' === $s[0]) )
        {
    ...
    protected static function match_repeats( $s )
    {
    ...
    protected static function chargroup( &$re_obj )
    {
        $sequence = array();
        $chars = array();
        $allchars = array();
    ...
    protected static function analyze_re( &$re_obj )
    {
        $word = ''; $wordlen = 0;
        $alternation = array();
        $sequence = array();
...
class RegexComposer
{
    ...
    {
        if ( null != $token )
            $this->ast[$this->level]->node[] = $escaped ? Regex::esc_re((string)$token, Regex::ESC) : (string)$token;
        return $this;
    }

    public function match( $token, $escaped=false )
    {
        return $this->token($token, $escaped);
    }
    ...
    {
        $this->ast[$this->level]->node[] = $multiline ? '['.Regex::ESC.'s'.Regex::ESC.'S]' : '.';
        return $this;
    }

    public function space( $positive=true )
    {
        $this->ast[$this->level]->node[] = !$positive ? Regex::ESC.'S' : Regex::ESC.'s';
        return $this;
    }

    public function digit( $positive=true )
    {
        $this->ast[$this->level]->node[] = !$positive ? Regex::ESC.'D' : Regex::ESC.'d';
        return $this;
    }

    public function word( $positive=true )
    {
        $this->ast[$this->level]->node[] = !$positive ? Regex::ESC.'W' : Regex::ESC.'w';
        return $this;
    }

    public function boundary( $positive=true )
    {
        $this->ast[$this->level]->node[] = !$positive ? Regex::ESC.'B' : Regex::ESC.'b';
        return $this;
    }
    ...
    public function repeat( $min, $max=null, $greedy=true )
    {
        ...
        $repeat = (null===$max || $min===$max? ('{'.(string)$min.'}') : ('{'.(string)$min.','.(string)$max.'}')) . (!$greedy ? '?' : '');
    ...
    {
        $this->ast[$this->level]->node[count($this->ast[$this->level]->node)-1] .= (!$greedy ? '??' : '?');
        return $this;
    }
    ...
    {
        $this->level++;
        $this->ast[] = (object)array('node'=> array(), 'type'=> Regex::T_ALTERNATION, 'flag'=> '', 'sequences'=> array());
        return $this;
    }
    ...
    public function or_( )
    {
        if ( (Regex::T_ALTERNATION === $this->ast[$this->level]->type) && !empty($this->ast[$this->level]->node) )
        {
            $this->ast[$this->level]->sequences[] = implode('', $this->ast[$this->level]->node);
            $this->ast[$this->level]->node = array();
        }
        return $this;
    }
    ...
    public function end( $n=1 )
    {
        ...
        while( $n-- )
        {
            ...
            if ( Regex::T_ALTERNATION === $type )
            {
                $sequences = $prev ? $prev->sequences : array();
                $part = empty($part) ? $sequences : array_merge($sequences, array(implode('', $part)));
            }
            if ( 0 < $this->level )
            {
                --$this->level;
